...
const SDL_FColor RED              = SDL_FColor { 1, 0, 0, 0.5f };
const SDL_FColor BLUE             = SDL_FColor { 0, 0.5f, 1, 0.6f };
...
std::unordered_map<K, SDL_FRect> g_KeyRects = {
    // Row 1
    { K::Escape, r( 24, 25 ) },
    { K::F1, r( 121, 25 ) },
    { K::F2, r( 176, 25 ) },
...
void renderKeyboard()
{
    ...
    if ( g_pKeyboardTexture )
    {
        ...
        SDL_GetTextureSize( g_pKeyboardTexture, &texW, &texH );
        int rtW = 0, rtH = 0;
        SDL_GetRenderOutputSize( g_pRenderer, &rtW, &rtH );
        float left = ( rtW - texW ) / 2;
        float top  = rtH - texH;

        SDL_FRect dst = { left, top, texW, texH };
        SDL_RenderTexture( g_pRenderer, g_pKeyboardTexture, nullptr, &dst );
        ...
        if ( g_pFont )
        {
            ...
            SDL_Surface* textSurface = TTF_RenderText_Blended( g_pFont, attribution, 0, color );
            if ( textSurface )
            {
                ...
                SDL_RenderTexture( g_pRenderer, textTexture, nullptr, &textRect );
...
void renderMouse()
{
    if ( g_pMouseTexture && g_pLMBTexture && g_pRMBTexture && g_pMMBTexture && g_pScrollUpTexture && g_pScrollDownTexture )
    {
        ...
        SDL_FRect dst = { g_MousePosition.x - texW / 2, g_MousePosition.y - texH / 2, texW, texH };
        SDL_RenderTextureRotated( g_pRenderer, g_pMouseTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
        if ( state.leftButton )
            SDL_RenderTextureRotated( g_pRenderer, g_pLMBTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
        if ( state.rightButton )
            SDL_RenderTextureRotated( g_pRenderer, g_pRMBTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
        if ( state.middleButton )
            SDL_RenderTextureRotated( g_pRenderer, g_pMMBTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
        if ( g_MouseStateTracker.scrollWheelDelta > 0 )
            SDL_RenderTextureRotated( g_pRenderer, g_pScrollUpTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
        if ( g_MouseStateTracker.scrollWheelDelta < 0 )
            SDL_RenderTextureRotated( g_pRenderer, g_pScrollDownTexture, nullptr, &dst, g_fMouseRotation, nullptr, SDL_FLIP_NONE );
    }
}
...
void drawCircle( SDL_FColor color, SDL_FPoint center, float radius )
{
    ...
    for ( float y = -radius; y <= radius; ++y )
    {
        ...
        SDL_RenderLine( g_pRenderer, center.x - dx, center.y + y, center.x + dx, center.y + y );
    }
}

void drawOutlineCircle( SDL_FColor color, SDL_FPoint center, float radius )
{
...
void drawRectangle( SDL_FColor color, SDL_FRect rect )
{
    SDL_SetRenderDrawColorFloat( g_pRenderer, color.r, color.g, color.b, color.a );
    SDL_RenderFillRect( g_pRenderer, &rect );
}

void drawOutlineRectangle( SDL_FColor color, SDL_FRect rect )
{
...
void renderGamepad( const Gamepad::State& state, float x, float y )
{
    ...
    {
        float texW = 0, texH = 0;
        SDL_GetTextureSize( g_pXBoxControllerTexture, &texW, &texH );
        SDL_FRect dst = { x, y, texW, texH };
        SDL_RenderTexture( g_pRenderer, g_pXBoxControllerTexture, nullptr, &dst );

        if ( state.buttons.leftShoulder )
            SDL_RenderTexture( g_pRenderer, g_pLeftBumperTexture, nullptr, &dst );

        if ( state.buttons.rightShoulder )
            SDL_RenderTexture( g_pRenderer, g_pRightBumperTexture, nullptr, &dst );
...
{
    SDL_SetRenderDrawColorFloat( g_pRenderer, PANEL_BACKGROUND.r, PANEL_BACKGROUND.g, PANEL_BACKGROUND.b, PANEL_BACKGROUND.a );
    SDL_SetRenderDrawBlendMode( g_pRenderer, SDL_BLENDMODE_BLEND );
    SDL_RenderFillRect( g_pRenderer, &panelRect );

    // Draw panel border
    SDL_SetRenderDrawColorFloat( g_pRenderer, PANEL_ACCENT.r, PANEL_ACCENT.g, PANEL_ACCENT.b, PANEL_ACCENT.a );
    constexpr int borderThickness = 8;
    for ( int i = 0; i < borderThickness; ++i )
    {
        SDL_FRect borderRect = { panelRect.x - i, panelRect.y - i, panelRect.w + 2 * i, panelRect.h + 2 * i };
        SDL_RenderRect( g_pRenderer, &borderRect );
    }
}
...
void renderMousePanel()
{
    ...
    if ( surf )
    {
        ...
        SDL_RenderTexture( g_pRenderer, tex, nullptr, &dstRect );
...
void renderTouch()
{
    ...
    for ( const auto& touch : state.touches )
    {
        ...
        if ( g_pFontMono )
        {
            ...
            SDL_Surface* surf   = TTF_RenderText_Blended( g_pFontMono, idText.c_str(), 0, color );
            if ( surf )
            {
                SDL_Texture* tex     = SDL_CreateTextureFromSurface( g_pRenderer, surf );
